...
> 当前进度：**阶段 4 光照与观感**（AO + 方向光、天空渐变、距离雾；BFS 真光照后置）。  
...
- **阶段 4 光照与观感**：顶点 AO、方向光、天空渐变、距离雾、透明远→近排序
- **阶段 5 物品与 UI**：9 格快捷栏、准星、底部 HUD、滚轮切格（背包后置）

### 主要缺口

- 无背包 / 存档 / Minecraft 式阳光+方块光 BFS
- 水：已有覆盖、流动、水位高度、双源无限水；水桶等可再扩展
...
| 4 | 破坏 / 放置方块 | 完成 |
| 5 | 碰撞 / 重力 / 第一人称玩家 | 完成 |
| 6 | 光照、生物、UI、存档… | **当前**（光照观感+UI 已做；待 BFS 光/存档/生物） |
...
1. **环境光遮蔽 / 加强顶点光照** ✅  
   方向 cardinal light（顶/侧/底）× 四角 AO；建网时采样邻接不透明块。
2. **天空盒 / 雾** ✅  
   `SkyRenderer` 视线渐变天空；chunk 着色器距离雾（`FOG_START`/`FOG_END`）遮盖加载边界。
3. **透明排序** ✅  
   先不透明后透明；水/花草按 Chunk 距离远→近混合。
4. **（进阶）阳光 + 方块光 BFS** — 后置；当前用顶点光近似。
...
1. **快捷栏（Hotbar）** ✅  
   `UI/Hotbar`：9 格默认方块；`1–9` 选格，滚轮循环；右键放置当前选中方块。
2. **准星 + 简单 HUD** ✅  
   `CrosshairRenderer` + `HotbarRenderer`（OpenGL 正交叠层；槽位框 + 图集图标 + 选中高亮）。
3. **背包** — 后置；当前用热键 / 快捷栏直接拿方块。
...
| 准星 / 快捷栏 HUD | `Renderer/CrosshairRenderer.cpp` / `HotbarRenderer.cpp` |
| 天空 / 雾 | `Renderer/SkyRenderer.cpp` / `chunk.frag` |
| 快捷栏数据 | `UI/Hotbar.*` |
