...
| **$-0.6 \le T < -0.2$ (亚寒带)** | $M < 0.0$<br>$M \ge 0.0$ | **Tundra** (苔原)<br>**Taiga** (针叶林) | 矮灌木，积雪<br>云杉木，阴冷多山 |
...
```
然后该柱 `y > height && y <= WATER_LEVEL` 灌水。寒带/亚寒带群系（极地雪原、苔原、针叶林）这些水的**最上一格**改成冰（`Ice`）。河心两格深，冰下仍是水。
...
由 **极地雪原 (SnowyPlains)、苔原 (Tundra) 和 针叶林 (Taiga)** 共同控制，不跟昼夜挂钩。

- **地表**：
  - `SnowyPlains` 与 `Tundra` 的 `y == height` $\rightarrow$ Snow（整格实心，可碰撞，材质为雪顶）。
  - `Taiga` 的 `y == height` $\rightarrow$ Grass，但周围的水域结冰。
- **水面结冰**：上述三个冷区群系若柱内有水，只把 **最上面那一格水** 换成 `Ice`，下面仍是水。
...
| **Tundra** | Snow | 雪 → 土 → 石 | 0.6 | 0.3 |
| **Taiga** | Grass | 草 → 土 → 石 | 0.9 | 0.8 |
...
| **Tundra** | 无 | 枯灌木 `1/40`，少量雪 |
| **Taiga** | 针叶木/云杉 (Spruce)，`base ≈ 1/15` | 蕨类 (Fern) `1/20`，枯灌木 |
...
### 建议新增方块
在 [`BlockId`](file:///Users/guokezhen/Desktop/计算机/计算机图形学/LearnOpengl/游戏/Minecraft/World/Block/BlockId.h) 尾部追加（已实现）：
...
```text
y > height:
    y <= WATER_LEVEL → 水；若（Tundra 或 Taiga 或 SnowyPlains）且这是最上一格水 → 冰
    否则空气
y == height:
    Ocean / Desert → 沙
    TemperateDesert → 沙（60%）或石头（40%）
    Forest / Grassland / Jungle / Taiga → Grass
    Savanna → SavannaGrass
    Tundra / SnowyPlains → Snow
height-3..height-1:
    Desert / Ocean → 沙
...
4. 寒带水面顶层结冰，冰下仍是水；苔原与雪原覆盖纯白雪块。
5. 热带草原草色偏黄偏秋意，树木极其稀少且树干偏橙色。
6. 针叶林中刷新出塔状云杉树与大片绿色蕨类。
7. 热带雨林中生长有高大的丛林巨木，植被极其浓密。
8. 陆地上有自然的弯曲河流，沙漠河流极稀，寒带河流结为冰河。
